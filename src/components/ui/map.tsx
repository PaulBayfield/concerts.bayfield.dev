...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  return createPortal(
    <div className={cn("bg-foreground text-background animate-in fade-in-0 zoom-in-95 rounded-md px-2 py-1 text-xs shadow-md", className)}>
      {children}
    </div>,
    container,
  );
...
function ControlButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      type="button"
      className={cn("hover:bg-accent dark:hover:bg-accent/40 flex size-8 items-center justify-center transition-colors", disabled && "pointer-events-none cursor-not-allowed opacity-50")}
      disabled={disabled}
    >
      {children}
    </button>
...
function MapClusterLayer<P extends GeoJSON.GeoJsonProperties = GeoJSON.GeoJsonProperties>({
  ...
  useEffect(() => {
    ...
    map.addSource(sourceId, {
      type: "geojson", data, cluster: true, clusterMaxZoom, clusterRadius,
    });
    ...
    map.addLayer({
      ...
      filter: ["!", ["has", "point_count"]],
      paint: {
        "circle-color": pointColor, "circle-radius": 8,
